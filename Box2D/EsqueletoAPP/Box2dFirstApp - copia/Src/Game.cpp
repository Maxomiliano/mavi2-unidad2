...
Game::Game(int ancho, int alto, std::string titulo)
{
    ...
    frameTime = 1.0f / fps;
    SetZoom(ancho, alto); // Configuraci�n de la vista del juego
...
void Game::SetZoom(int ancho, int alto)
{
    View camara;
    // Posicionamiento y tama�o de la vista
    camara.setSize(ancho, alto);
    camara.setCenter(ancho / 2 , alto / 2);
...
void Game::InitPhysics()
{
    ...
    groundBody->SetTransform(b2Vec2(400.0f, 580.0f), 0.0f);

    b2Body* leftWallBody = Box2DHelper::CreateRectangularStaticBody(phyWorld, 20, 800);
